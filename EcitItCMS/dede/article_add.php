...
else if($dopost=='save')
{
    ...
    if(!$dsql->ExecuteNoneQuery($query))
    {
        ...
        ShowMsg("把数据保存到数据库主表 `#@__archives` 时出错，请把相关信息提交给EcitITCMS官方。".str_replace('"','',$gerr),"javascript:;");
        exit();
    }
    ...
    if(!$dsql->ExecuteNoneQuery($query))
    {
        ...
        $dsql->ExecuteNoneQuery("Delete From `#@__archives` where id='$arcID'");
        $dsql->ExecuteNoneQuery("Delete From `#@__arctiny` where id='$arcID'");
        ShowMsg("把数据保存到数据库附加表 `{$addtable}` 时出错，请把相关信息提交给EcitITCMS官方。".str_replace('"','',$gerr),"javascript:;");
        exit();
    }
    //生成HTML
