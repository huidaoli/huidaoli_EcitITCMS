 ...
 *
 * @version        $Id: dedesqli.class.php 1 15:00 2011-1-21 tianya $ * @package        EcitITCMS.Libraries
 ...
 *
 * @package        DedeSqli
 * @subpackage       EcitITCMS.Libraries
 * @link           http://www.ecit-it.com
 */
class DedeSqli
{
    ...
    function Open($pconnect=FALSE)
    {
        ...
        else
        {
            ...
            mysqli_errno($this->linkID) != 0 && $this->DisplayError('EcitITCMS错误警告： 链接('.$this->pconnect.') 到MySQL发生错误');
        ...
        if(!$this->linkID)
        {
            $this->DisplayError("EcitITCMS错误警告：<font color='red'>连接数据库失败，可能数据库密码不对或数据库服务器出错！</font>");
            exit();
        }
		$this->isInit = TRUE;
...
EOT;
    ...
    function DisplayError($msg)
    {
		...
		{
			$emsg = '';
			$emsg .= "<div><h3>EcitITCMS Error Warning!</h3>\r\n";
			$emsg .= "<div><a href='http://www.ecit-it.com' target='_blank' style='color:red'>Technical Support: http://www.ecit-it.com</a></div>";
